...
default_dir = r'D:\Thinh\Ca_nhan\hus\project python'
...
result.to_excel(os.path.join(default_dir,'result_thinh1.xlsx'), index=False)
...
feature_importances.to_excel(os.path.join(default_dir,'feature_importance_thinh1.xlsx'), index=False)

# save model
joblib.dump(clf, os.path.join(default_dir('lightgbm_final.p')))
# load model
gbm_pickle = joblib.load(os.path.join(default_dir('lightgbm_final.p')))
#check = gbm_pickle.predict_proba(Z_TEST, num_iteration=clf.best_iteration_)[:, 1]
